...
function PLA(opt, lb, ub, prob, data, storage; test, id, output_file="pla_results.csv")
    # Check if optimized parameters are within bounds
    if any(opt .< lb) || any(opt .> ub)
        @warn "Optimized parameters for ID $id are out of bounds. Skipping PLA."
        results = DataFrame(
            ID = fill(id, 4),
            Parameter = ["k1", "k5", "k6", test == "CGM" ? "tauG" : "N/A"],
            Lower_Bound = fill(NaN, 4),
            Upper_Bound = fill(NaN, 4),
            Identifiability = fill("Out of bounds", 4)
        )
    ...
    for i in 1:num_params
        ...
        try
            ...
            @warn "Error in PLA for parameter $param_name of ID $id: $e"
...
function gen_results(;test="OGTT")
    ...
    for i in 1:n
        ...
        if test == "OGTT"
            storage = fill(NaN, 1, 9)  # 3 parameters * 3 values each (lower, estimate, upper)
        else
        ...
        results = PLA(opt_sol.u, lb, ub, _prob, grp_data[i], storage; 
                      test=test, id=subject_id, output_file="pla_results_$(test).csv")
        
        # Store PLA results
        for row in eachrow(results)
            param = row.Parameter
            if param in ["k1", "k5", "k6", "tauG"]
                grp_estimates[i][1, Symbol(param, "_l")] = row.Lower_Bound
                grp_estimates[i][1, Symbol(param)] = opt_sol.u[findfirst(==(param), ["k1", "k5", "k6", "tauG"])]
                grp_estimates[i][1, Symbol(param, "_r")] = row.Upper_Bound
            end
        end
