...
function gen_results(;test="OGTT")
    ...
    est_p = DataFrame(k1=Vector{Union{Float64, Missing}}(missing, nrow(estimates)), # modifications to accomodate missing values
    ...
    # Load k6 estimates
    k6_estimates = load_k6_estimates() # load k6 estimates from the k6_estimates.csv file
    ...
    # p_adj, lb, ub, p_fix, u0, tspan = set_conditions(test) # commented out as we are using personalized k6
