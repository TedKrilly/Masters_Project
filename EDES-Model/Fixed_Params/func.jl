...
function set_conditions(test)
    ...
    if test == "OGTT"
        ...
        print("Set_conditions : Fixed_k6 variant should always be OGTT type metric")
        # p_adj = [
        #     1.35e-2,  #k1
        #     3.80e-3,  #k5
        #     5.82e-1,  #k6
    ...
    p_fixed = [
        ...
        1.00e-3,  #k4
        3.80e-3,  #k5
        5.82e-1,  #k6 should make this a variable 'fixed_k6
        1.15,     #k7
        4.71,     #k8
        1.08e-2,  #k9
        1.35,     #sigma /index 10
    ...
    tspan = (0.0, 240.0)    # Change from 130.0 to 240.0
...
function PLA(opt, lb, ub, prob, data, storage; test)
    ...
    if test == "OGTT"
        ...
        print("PLA : Fixed_k6 variant should always be OGTT type metric")
        # with delay 
...
function bands(opt, sol, lb, ub, prob, data, ts_sim; test="OGTT")
    ...
    if test == "OGTT"
        ...
        print("bands : Fixed_k6 variant should always be OGTT type metric")
        # with delay 
        # scan_bounds = [(1e-10, ub[1]*100), (1e-10, ub[2]*100), (1e-10, ub[3]*10), (1e-9, 25.0)]
        # theta_bounds = [(0.0, ub[1]*200), (0.0, ub[2]*200), (0.0, ub[3]*20), (1e-10, 30.0)]
    end
...
function plot_bands(sim, data, glbands, gubands, ilbands, iubands; test)
    ...
    if test == "CGM"
        print("plot_bands : Only OGTT should be used")
        u_g = Array(sim)[4,1:131];
    else
        u_g = Array(sim)[2,1:241]; # Changed from 1:131 to 1:241
    end
    u_i = Array(sim)[3,1:241]; # Changed from 1:131 to 1:241
    ...
    p[1] = Plots.plot(Float64.(0:240), u_g, xlabel="time", ylabel="glucose, mmol/L", labels="Sim", line=3, # Changed from 0:130 to 0:240
    ribbon = (u_g-glbands, gubands-u_g), fc=:orange, fa=0.3);
    Plots.scatter!(ts_glu, Vector(data_glu), labels="data", legend=:outertopright);
    p[2] = Plots.plot(Float64.(0:240), u_i, xlabel="time", ylabel="insulin, mU/L", labels="Sim", line=3, # Changed from 0:130 to 0:240
...
function gen_results(;test="OGTT")
    data = CSV.read("mmt_glucose_and_insulin_data.csv", DataFrame);
    ...
    if test == "OGTT"
        ...
        print("gen_results_estimates : Fixed_k6 variant should always be OGTT type metric")
    ...
    for i in 1:n
        ...
        sol = Array(sol_opt);  # Convert solution to Array

        # store simulated glucose and insulin
        ts_data = parse.(Int, names(grp_data[i])[4:end])
        if test == "OGTT"
            grp_simulated[i][1,4:end] = sol[2,ts_data .+ 1];
            grp_estimates[i][1,[:k1, :k5]] = opt_sol.u;
        else
            print("gen_results_simulated : Fixed_k6 variant should always be OGTT type metric")
            # grp_simulated[i][1,4:end] = sol[4,ts_data .+ 1];
            # grp_estimates[i][1,[:k1, :k5, :k6, :tauG]] = opt_sol.u;
        end
        grp_simulated[i][2,4:end] = sol[3,ts_data .+ 1];

        # store residuals
        if test == "OGTT"
            grp_residuals[i][1, 4:end] = Vector(Array(grp_data[i][isequal.(grp_data[i].metab,"gluc"), 4:end])[1,:]) - Vector(grp_simulated[i][1,4:end]) #change to Vector
        else
            print("gen_results_residuals : Fixed_k6 variant should always be OGTT type metric")
            # grp_residuals[i][1, 4:end] = Array(grp_data[i][isequal.(grp_data[i].metab,"gluc"), 4:end])[1,:] - grp_simulated[i][1,4:end]
        end
        grp_residuals[i][2, 4:end] = Vector(Array(grp_data[i][isequal.(grp_data[i].metab,"ins"), 4:end])[1,:]) - Vector(grp_simulated[i][2,4:end]) #change to Vector    
