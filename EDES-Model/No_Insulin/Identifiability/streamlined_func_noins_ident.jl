...
function loss(p, prob, data)
    # modified version of the loss function that only takes into account
    # the CGM glucose data fit.
    ...
    if any((s.retcode != :Success for s in sol))
        ...
        loss = 100*loss_g  # Only consider glucose error
    end
    return loss
end
...
function update_conditions!(p_adj, p_fix, u0, data; test="OGTT")
    ...
    if size(data, 1) < 2
        error("Data does not have enough rows to access required elements.")
    end
...
function PLA(opt, lb, ub, prob, data, storage; test, id, output_file="pla_results_CGM_noins.csv")
    if test == "CGM"
        ...
        param_names = ["k1", "k5", "k6", "tauG"]
    else
        error("This PLA function is designed for CGM data only")
    end
    ...
    for i in 1:num_params
        ...
        try
            ...
            push!(results, (id, param_name, opt[i], lower_bound, upper_bound, identifiability, ""))
        catch e
            error_info = sprint(showerror, e, catch_backtrace())
            push!(results, (id, param_name, opt[i], NaN, NaN, "Error in calculation", error_info))
            storage[1, i*3-2] = NaN
            storage[1, i*3-1] = opt[i]
            storage[1, i*3] = NaN
        end
...
function gen_results(;test="CGM")
    data = CSV.read("cleaned_test.csv", DataFrame);
    if test == "OGTT"
        # select OGTT data
    ...
    elseif test == "CGM"
        # Check if there's any CGM data
        if !any(data.test .== "CGM")
            println("No CGM data available for this ID. Skipping CGM analysis.")
            return
        end
    ...
    res_ts = DataFrame(fill(missing, nrow(residuals), 131), :auto)
    ...
    for i in 1:n
        ...
        ## Added in by me
        # Check if the group has missing data
        if any(ismissing, eachrow(grp_data[i]))
            println("Skipping group ", grp_data[i][1,:ID], grp_data[i][1,:Condition], " due to missing data")
            continue
        end

        # Check if the group has enough rows for CGM data
        if test == "CGM" && size(grp_data[i], 1) < 2
            println("Skipping group ", grp_data[i][1,:ID], grp_data[i][1,:Condition], " due to insufficient CGM data")
            continue
        end
        ...
        storage = fill(NaN, 1, 12)  # 4 parameters * 3 values each (lower, estimate, upper)

        # Add PLA here
        subject_id = grp_data[i][1,:ID]
        results = PLA(opt_sol.u, lb, ub, _prob, grp_data[i], storage; 
                      test=test, id=subject_id, output_file="pla_results_CGM_noins.csv")
        
        # Store PLA results
        grp_estimates[i][1, [:k1_l, :k1, :k1_r, :k5_l, :k5, :k5_r, :k6_l, :k6, :k6_r, :tauG_l, :tauG, :tauG_r]] = storage

        # Commenting out PLA and plotting
