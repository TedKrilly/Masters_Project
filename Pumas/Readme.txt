...
@param block: used to define model parameters
    - four key parameters (k1, k5, k6, τ_g) - the same parameters as EDES model
    - assume LogNormal distributions

@random block: random effects for each parameter 

@covariates: covariate data type to distinguish between OGTT and CGM data, more flexibility in handling different types of glucose measurements

@preprocessing: pre block is used for defining fixed parameters,, constants and applying random effects, explicitly calculate c11 for clarity before the model simulation.
...
To handle CGM or OGTT Gi' calculates in all cases but used selectively

@derived block: 
glucose is set to Gi if data_type is "CGM", otherwise it is set to Gpl.
Conditional statememtn to select either Gi or Gpl based upon data type, 

@error block: Different error models for CGM and OGTT data, why? If data_type is "CGM", the glucose measurement error is modeled as a normal distribution with a standard deviation proportional to the glucose level.
